...
void handle_http(int client, struct sockaddr_in client_addr)
{
  ...
  if (a.method == "GET")
  {
    ...
    {
      response = {"HTTP/1.1 200 OK", "text/plain", {{"Content-Length", std::to_string(a.headers["User-Agent"].length())}}, a.headers["User-Agent"]};
    }
    ...
    else if (a.path.substr(0, 7) == "/files/")
    {
      std::string fileName = a.path.substr(7);
      std::ifstream inputFile(dir + fileName);
      if (!inputFile.is_open())
      {
        response = {"HTTP/1.1 404 Not Found", "text/plain", {}, "Not Found"};
      }
      else
      {
        std::string line;
        ...
        response = {"HTTP/1.1 200 OK", "application/octet-stream", {{"Content-Length", std::to_string(content.length())}}, content};
      }
    }
    else
    {
  ...
  {
    response = {"HTTP/1.1 405 Method Not Allowed", "text/plain", {}, "Method Not Allowed"};
  }
...
int main(int argc, char **argv)
{
  ...
  {
    std::cerr << "Failed to bind to port f\n";
    return 1;
  }
